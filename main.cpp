...
int main(int argc, char *argv[]) {
  ...
  cout << correct_T << "," << total << ";" << endl << correct_NT << "," << total << ";" << endl;
  for(int i = 0; i < 8; i++){
    if(i == 2) continue;
    cout << singleBits[i]->get_correct() << "," << total << "; ";
    //cout << (singleBits[i]->get_correct() == single_actual[i]) << ", ";
  }
  cout << endl;
  for(int i = 0; i < 8; i++){
    if(i == 2) continue;
    cout << doubleBits_independent[i]->get_correct() << "," << total << "; ";
    //cout << (doubleBits_independent[i]->get_correct() == double_actual[i]) << ", ";
  }
  cout << endl;
  for(int i = 0; i < 9; i++){
    cout << gshare_independent[i]->get_correct() << "," << total << "; ";
    //cout << (gshare_independent[i]->get_correct() ==  gshare_actual[i]) << ", ";
  }
  
  cout << endl << tourny->get_correct() << "," << total << ";" << endl;
  return 0;
}
